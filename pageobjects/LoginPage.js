...
class LoginPage {
  constructor(page) {
    ...
    this.subTopicTextBox = page.locator("div[class*= 'Image-sc']");
  ...
  async LaunchURLAndClickOn3DPanel(panelnumb) {
    ...
    await expect.soft(this.ToggleChat, "Expecting the Toggle Chat button to be enabled [clickable]").toBeEnabled(); // added June 16th.
    await this.page.evaluate((panel) => {
      window.blazeIT_Susanoo.ctx.susanoo.on3DObjectClicked(panel);
    }, panelnumb);
  }
async waitForFeedContentToStabilize() {
  await this.page.evaluate(() => {
    return new Promise(resolve => {
      const el = document.querySelector("div[class*= 'StyledLayout-sc']"); // 🔁 Use your real selector
      if (!el) return resolve(); // Exit early if not found

      const observer = new MutationObserver(() => {
        observer.disconnect();
        setTimeout(resolve, 2000); // Add slight buffer after mutation
      });
