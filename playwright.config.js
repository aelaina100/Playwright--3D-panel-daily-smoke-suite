...
const Config = ({
  ...
  use: {
   ...
   // 🚨 This ensures only one worker runs tests — i.e., no parallel execution across files (Added seperatley)
   //My own notes: setting 'workers' to 1 ensures that local PC browsers (defined above) & BrowserStack stuff (defined in Project),
    // do not run parallelly. The final outcome is that = each single testcases on PC and browser stack run sequentially.
  workers: 1, 
  projects: [ // to match the platforms defined in browserstack.yaml
    {
    ...
    {
      name: 'playwright-webkit@latest:OS X Ventura@browserstack',
