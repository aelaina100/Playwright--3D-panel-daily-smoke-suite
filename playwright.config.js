...
const Config = ({
  testDir: './tests',
 // retries:1, // failed test will run again for 2 times. If it passes on the 1st re-run, then it won't execute one more time.Report only shows such testcase
  ...
  projects: [ // One can specify, in the command line, which browser to run the scripts on
     ...
     
     {
        ...
        use: {
                ...
                headless: false,    // Playwright, by default, runs in headless mode (=true)
                screenshot: 'on',   // ''only-on-failure','off'
                video:'on', // 'retain-on-failure' 'on-first-retry' -Recording a video only when re-trying a test for the first time.
                ...
                trace: 'on', //to enable video recording on failure
...
export default defineConfig({ 
  reporter: [  //I suspect this is for Slack. double-check !
    ['html'],
