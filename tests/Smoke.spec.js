...
test.only(`@Smoke Validating: [AN INTRO TO CINCO] feed page`, async ({ browser }) => {
...
await test.step("Clicking on the last image (if it exists) in the feed page", async () => {
  try {
    await expect(loginPage.feedImages.first()).toBeVisible({ timeout: 5000 });

    const count = await loginPage.feedImages.count();
    console.log(`Number of images found: ${count}`);

    if (count > 0) {
      const lastImage = await loginPage.feedImages.nth(count - 1);
      await lastImage.scrollIntoViewIfNeeded();
      await lastImage.click();
    }
  } catch (error) {
    console.log("No images found or visible within the timeout window. Skipping click.");
    // Optionally, you could add a soft assertion or log to your test reporting framework here
  }
});
    ...
    //await loginPage.feedCloseButton.click({ force: true });
    //better option below ?
    const handle = await loginPage.feedCloseButton.elementHandle();
if (handle) {
