...
for (let i = 0; i < expected_count_subTopics; i++) { 
  ...
  await expect(
    loginPage.subTopicImages.nth(i), 
  ...
  await loginPage.subTopicImages.nth(i).click();
});
 
await test.step("Ensuring Feed Page Readiness via Last Image Right-Click (If present)", async () => {
  await loginPage.rightClickOnlastImageFeedpage();
});
    await expect.soft(
    ...
    await loginPage.nativeClickFeedClose();
    });
...
test(`@Smoke Validating: [Cinco's AI-Powered Innovation] feed page`, async ({ browser }) => { 
