...
const dataset = JSON.parse(JSON.stringify(require("../utils/loggingTestData.json"))); // converting this json into a Javascript object so it is easier to access
...
test('Smoke Test: Ensuring app URL is not brocken', async ({ page }) => {   // 'async' & 'await' keywords go hand in hand
 ...
 await page.goto(dataset.URL); // This URL has to be set globally (will see how to do so and where to set it)
...
test('Smoke Test: Validating that [ENTER] button is visible & Enabled', async ({ page }) => {
  ...
  //
  // Navigate to cinco's url:
  await page.goto(dataset.URL); // This URL has to be set globally (will see how to do so and where to set i
