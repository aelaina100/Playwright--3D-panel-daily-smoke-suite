...
test('Smoke Test: Ensuring app URL is not brocken', async ({ page }) => {   // 'async' & 'await' keywords go hand in hand
 ...
 await page.goto(urldataset.URL); // This URL has to be set globally (will see how to do so and where to set it)
  ...
  await page.close(); // prevents you from taking a photo when screenshot = 'on'
...
test('Smoke Test: Validating that [ENTER] button is visible & Enabled', async ({ page }) => {
  ...
  //
  // Navigate to cinco's url:
  await page.goto(urldataset.URL); // This URL has to be set globally (will see how to do so and where to set i
...
// April 17th works
test('Validate that the 6 panels page is accessible to users', async ({browser})=> // not taking a photo when passed, why ?
{
...
{
  test.only(`Verify that Panel ${data.panelnumb}: [An Intro to Cinco] is accessible to users`, async ({browser }) => {
