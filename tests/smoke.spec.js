...
test('🚬@Smoke Validating the Entry screen of the cinco app', async ({ page }) => {
  ...
  await test.step("Navigating to the application URL: " + APP_URL, async () => {
  ...
  await expect.soft(page, `Ensuring the page remains on the expected URL: "${APP_URL}" with no unexpected redirects`).toHaveURL(urldataset.URL);
...
test('🚬@Smoke Validating the 6-panel page', async ({ browser }) => {
  ...
  await test.step("Navigating to the application URL: [" + APP_URL + "] and accessing the 6-panel application interface", async () => { 
  await loginPage.launchAppAndAccessPanelPage();
  });
  await expect.soft(page, `Expecting the URL of the 6-panel page to be: "${APP_URL}"`).toHaveURL(APP_URL); 
...
for (const [index, name] of panelNames.entries()) {  
  test(`🚬@Smoke Verify that ${index + 1} [${name}] is accessible and loads correctly for users`, async ({ browser }) => {
    ...
    await loginPage.ClickOn3DPanelNumber(name);
    });
...
for (let i = 0; i < actual_subTopicCount; i++) { 
  ...
  await page.waitForTimeout(2000); 
    ...
    await feedCloseButton.click();  
    });
    await expect(feedCloseButton, "The feedpage has closed after clicking on its close (X) button").not.toBeAttached(); 
    await page.waitForTimeout(2000); 
